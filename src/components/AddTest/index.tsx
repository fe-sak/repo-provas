...
const AddTest = () => {
  ...
  const [disciplines, setDisciplines] = useState<api.disciplines>([]);
  const [teachers, setTeachers] = useState<api.teachers>([]);
  ...
  const fetchData = useCallback(async () => {
    const { data: categories }: { data: api.categories } = await api.getCategories(auth);

    setCategories(categories);

    const { data: disciplines }: { data: api.disciplines } = await api.getDisciplines(auth);

    setDisciplines(disciplines);

    const { data: teachers }: { data: api.teachers } = await api.getTeachers(auth);
      ...
      const filteredTeachers = teachers
        .filter((teacher) =>
          teacher.disciplinesTeachers.find(
            (disciplineTeacher) => disciplineTeacher.discipline.name === formData.discipline
          )
        )
  ...
  const handleSubmit: FormEventHandler<HTMLDivElement> = (event) => {
    ...
    const selectedTeacher = teachers.find((teacher) => teacher.name === formData.teacher);
  ...
  if (categories === null || disciplines === null || teachers === null) return <LoadingSpinner />;
  ...
  return (
    <Box sx={{ marginLeft: '5px', marginRight: '5px' }}>
      ...
      <Box onSubmit={handleSubmit} component='form' sx={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        <TextField
          name='name'
          fullWidth
          variant='outlined'
        ...
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} label='Categorias' variant='outlined' />}
        />
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} label='Disciplinas' variant='outlined' />}
        />
        <Autocomplete
          ...
          renderInput={(params) => <TextField {...params} label='Professores' variant='outlined' />}
        />
        <Button
          disabled={!(formData.name && formData.pdfUrl && formData.category && formData.discipline && formData.teacher)}
          type='submit'
          variant='contained'
          sx={{
